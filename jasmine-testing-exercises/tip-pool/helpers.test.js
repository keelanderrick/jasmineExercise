describe("Helpers test (with setup and tear-down", function() {
    beforeEach(function () {
        billAmtInput.value = 100;
        tipAmtInput.value = 20;
        submitPaymentInfo();
    })

    it('should sum total tip amount of all payments on sumPaymentTotal()', function () {
        // first test the current single payment
        expect(sumPaymentTotal('tipAmt')).toEqual(20);

        // test that the sum works by adding a new payment with different values
        billAmtInput.value = 200;
        tipAmtInput.value = 40;
        submitPaymentInfo();

        expect(sumPaymentTotal('tipAmt')).toEqual(60);
    });

    it('should sum total bill amount of all payments on sumPaymentTotal()', function () {
        // first test the current single payment
        expect(sumPaymentTotal('billAmt')).toEqual(100);

        // test that the sum works by adding a new payment with different values
        billAmtInput.value = 200;
        tipAmtInput.value = 40;
        submitPaymentInfo();

        expect(sumPaymentTotal('billAmt')).toEqual(300);
    });

    it('should sum total bill amount of all payments on sumPaymentTotal()', function () {
        // first test the current single payment
        expect(sumPaymentTotal('tipPercent')).toEqual(20);

        // test that the sum works by adding a new payment with different values
        billAmtInput.value = 200;
        tipAmtInput.value = 40;
        submitPaymentInfo();

        expect(sumPaymentTotal('tipPercent')).toEqual(40);
    });

    it('should sum tip percent of a single tip on calculateTipPercent()', function() {
        // verify that calculate tip percent works across a couple examples
        expect(calculateTipPercent(100,23)).toEqual(23);
        expect(calculateTipPercent(10,1)).toEqual(10);
    });

    it('should generate delete td and append to tr on appendDeleteBtn', function() {
        // verify that appendDeleteBtn works (creates a button) by creating a tr and appending it
        let newTr = document.createElement('tr');
        appendDeleteBtn(newTr);
        
        expect(newTr.children.length).toEqual(1);
        expect(newTr.firstChild.innerText).toEqual('X');
    });

    afterEach(function () {
        // reset all values used
        billAmtInput.value = '';
        tipAmtInput.value = '';
        paymentTbody.innerHTML = '';
        summaryTds[0].innerHTML = '';
        summaryTds[1].innerHTML = '';
        summaryTds[2].innerHTML = '';
        serverTbody.innerHTML = '';
        allPayments = {};
        paymentId = 0;
    });
});